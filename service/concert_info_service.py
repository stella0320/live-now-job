...
class ConcertInfoService():
    ...
    def get_column_name(self):
        ...
        table = Table('concert_info', metadata, autoload=True, autoload_with=engine)
    ...
    def update_column_values_by_id(self, update_values, primary_key_value):
        if primary_key_value and update_values:
            try:
                stmt = update(ConcertInfo).where(ConcertInfo.concert_info_id == primary_key_value).values(update_values)
