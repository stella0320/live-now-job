...
if __name__ == '__main__':
    ...
    client = OpenAI(
        api_key="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",
    )
    ...
    completion = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You will be provided with unstructured chinese html text, and your task is to parse it into json format like this {'concert_title': string, 'concert_time': list of YYYY-MM-E hh:mm, 'sell_ticket_time': list of  YYYY-MM-EE hh:mm, concert_singer: list of string ,concert_location:string}."},
            {"role": "user", "content": concert_content}
            # {"role": "assistant", "type":"json_object"}
        ],
        temperature=0,
        max_tokens=500
    )
